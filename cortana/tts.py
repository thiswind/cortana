...
def stream_text_to_voice(voice_id: str, text: str):
    response_stream = make_api_request(
        'POST',
        ApiType.ELEVENLABS,
        TTS_STREAM.format(voice_id=voice_id),
        data={
            "text":
            "Oh, shampoo, that magical elixir that we all slather on our hair without a clue as to how it actually works. It's like putting faith in a wizard who claims to have a secret potion that will make your hair shine like diamonds.",
            "voice_settings": {
                "stability": 0.5,
...
def get_non_premade_voices(
        voices: list[dict[Any, Any]]) -> list[dict[Any, Any]]:
    return [
...
def find_voice_by_name(voices: list[dict[Any, Any]],
                       name: str) -> dict[Any, Any] | None:
    return next((v for v in voices if name.lower() in v['name'].lower()), None)


def select_pyaudio_output_device(devices: list[Any],
                                 device_index: int = 0
                                 ) -> dict[Any, Any] | None:
...
def play_response(response_data, device: Any):
    ...
    stream = p.open(format=pyaudio.paInt16,
                    ...
                    frames_per_buffer=DOWNLOAD_BLOCK_SIZE)
    print('Playing audio...')
    audio_segment = AudioSegment.from_file(io.BytesIO(response_data),
                                           format='mp3')
...
def tts_loop(text: str | None = None):
    ...
    voices = get_voices()
    # voice = find_voice_by_name(get_non_premade_voices(voices),
    #                            os.environ.get('ELEVENLABS_VOICE_NAME', ''))
    voice = voices[4]
    if not voice:
